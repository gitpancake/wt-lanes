...
# 2) non-lane dir -> state set, kill path never engages.
...
# 3) lane dir, no agent-pid -> state set, manual-kill warning, no watcher.
...
# 6) DONE wins: lane declared done before the grace expired -> no kill.
spawn_fake
